...
Struct_uniform* VK_uniform::create_uniform_buffer(string name, size_t size, int binding){
  ...
  //---------------------------
    ...
    vkMapMemory(vk_struct->device.device, uniform->mem, 0, size, 0, &uniform->mapped);
...
void VK_uniform::clean_uniform(Struct_binding* binding){
  ...
  for(int i=0; i<binding->vec_uniform.size(); i++){
    ...
    vkDestroyBuffer(vk_struct->device.device, uniform->buffer, nullptr);
    vkFreeMemory(vk_struct->device.device, uniform->mem, nullptr);
  }
