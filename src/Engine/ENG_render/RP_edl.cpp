...
void RP_edl::create_subpass(Struct_vk_renderpass* renderpass){
  //---------------------------

  Struct_vk_subpass* subpass = new Struct_vk_subpass();
  subpass->target = "shader";
  subpass->draw_task = [this](Struct_vk_subpass* subpass){RP_edl::draw_edl(subpass);};
...
//Draw function
void RP_edl::draw_edl(Struct_vk_subpass* subpass){
  //---------------------------
...
void RP_edl::update_descriptor(Struct_vk_subpass* subpass){
  //---------------------------

  Struct_vk_renderpass* renderpass_scene = vk_engine->get_renderpass(0);
  Struct_vk_framebuffer* frame_scene = renderpass_scene->framebuffer;
...
void RP_edl::draw_command(Struct_vk_subpass* subpass){
  //---------------------------

  EDL_param* edl_param = edl_shader->get_edl_param();
  Struct_vk_pipeline* pipeline = subpass->get_pipeline();
