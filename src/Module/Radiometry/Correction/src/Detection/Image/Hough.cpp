...
Hough::Hough(rad::cor::Node* node_correction){
  //---------------------------
...
//Main function
void Hough::detect_circle(cv::Mat& image, std::shared_ptr<utl::base::Image> output){
...
void Hough::compute_hough_circle(cv::Mat& image){
  ...
  for(int i=0; i<circles.size(); i++){
    rad::cor::structure::Circle circle;
...
//Draw function
void Hough::draw_detected_circle(cv::Mat& image, std::shared_ptr<utl::base::Image> output){
  //---------------------------
...
void Hough::draw_all_circle(cv::Mat& image, std::shared_ptr<utl::base::Image> output){
  if(image.empty()) return;
  //------------------------
...
void Hough::draw_best_circle(cv::Mat& image, std::shared_ptr<utl::base::Image> output){
  if(image.empty()) return;
  //------------------------
