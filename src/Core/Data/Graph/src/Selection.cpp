...
Selection::Selection(dat::gph::Node* node_graph){
  ...
  this->gph_graph = node_graph->get_gph_graph();
  this->gph_publisher = node_graph->get_gph_publisher();
  this->elm_set = node_element->get_dat_set();
...
void Selection::select_element(std::shared_ptr<utl::base::Element> element){
  if(!element || !element->is_selectable) return;
  //----------------------------

  gph_struct->selection = element;
  gph_publisher->notify_selection(element);

  //If an entity, make it active
  if(auto entity = std::dynamic_pointer_cast<dat::base::Entity>(element)){
    elm_set->active_entity(entity->set_parent.lock(), entity);
  }

  //----------------------------
}
void Selection::select_next_element(){
  std::shared_ptr<dat::base::Set> set_graph = gph_graph->get_set_graph();
  std::shared_ptr<utl::base::Element> element = gph_struct->selection.lock();
  //----------------------------

  if(!element){
    this->select_element(set_graph);
    return;
  }
  ...
  if(auto set = std::dynamic_pointer_cast<dat::base::Set>(element)){
    ...
    if(!set->list_entity.empty()){
      this->select_element(*set->list_entity.begin()); // Select first entity in the set
      return;
    }
    if(!set->list_subset.empty()){
      this->select_element(*set->list_entity.begin()); // Select first subset in the set
      return;
    }
    ...
    if(auto set_parent = set->set_parent.lock()){
      ...
      if(it != set_parent->list_subset.end()){
        ...
        if(it != set_parent->list_subset.end()){
          this->select_element(*it); // Select the next subset in the parent set
          return;
        }
      }
  ...
  // Attempt to cast to dat::base::Entity
      ...
      if(auto set_parent = entity->set_parent.lock()){
          ...
          if(it != set_parent->list_entity.end()){
              ...
              if(it != set_parent->list_entity.end()){
                this->select_element(*it); // Select the next entity in the parent set
                return;
              }
          }
          // If no next entity in the parent set, check subsets
          if(!set_parent->list_subset.empty()){
            this->select_element(*set_parent->list_subset.begin()); // Select first subset in the parent set
            return;
          }
