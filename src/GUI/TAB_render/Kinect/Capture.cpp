  ...
  if(ImGui::BeginTable("database_view", 3, flags)){
    if(k4a_swarm->get_nb_device() == 0){
      ...
      for(int i=0; i<list_device.size(); i++){
        ...
        if(k4a_device->device.is_virtual) continue;
...
void Capture::configuration_device(){
  ...
  ImGui::Separator();
  string device_header = "Device S/N: " + k4a_device->device.serial_number;
  ...
  if(k4a_device->color.config.enabled){
    ...
    if(ImGui::TreeNode("Color control")){
      ...
      if(ImGui::RadioButton("50Hz", &power_frequency, 0)){
        k4a_device->color.config.power_frequency.value = "50Hz";
      }
      ImGui::SameLine();
      if(ImGui::RadioButton("60Hz", &power_frequency, 1)){
        k4a_device->color.config.power_frequency.value = "60Hz";
      }
...
void Capture::configuration_general(){
  ...
  ImGui::Checkbox("Disable streaming LED", &k4a_device->synchro.disable_streaming_indicator);
