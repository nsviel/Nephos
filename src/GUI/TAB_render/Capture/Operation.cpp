...
void Operation::colorization(){
  ...
  ImGui::ColorEdit4("##unicolor_choice", (float*)&k4n_device->cloud.object->unicolor, flags);
  ImGui::SameLine();
  ...
  if(k4n_device->cloud.color_mode == 3){
    ...
    if(k4n_device->cloud.heatmap_mode == 1){
      float min = k4n_device->cloud.object->min.z + 10;
      float max = k4n_device->cloud.object->min.z + 10;
