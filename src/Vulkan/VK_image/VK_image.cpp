...
void VK_image::clean_image(Struct_image* image){
  ...
  vkDestroySampler(vk_struct->device.device, image->sampler, nullptr);

  if(image->view != nullptr)
  vkDestroyImageView(vk_struct->device.device, image->view, nullptr);
  vkDestroyImage(vk_struct->device.device, image->image, nullptr);
  vkFreeMemory(vk_struct->device.device, image->mem, nullptr);
...
void VK_image::create_image_view(Struct_image* image){
  ...
  VkResult result = vkCreateImageView(vk_struct->device.device, &viewInfo, nullptr, &image->view);
...
VkImageView VK_image::create_image_view(VkImage image, VkFormat format, VkImageAspectFlags aspectFlags){
	...
	VkImageView imageView;
	if (vkCreateImageView(vk_struct->device.device, &viewInfo, nullptr, &imageView) != VK_SUCCESS)
	{
		throw std::runtime_error("failed to create texture image view!");
	}
...
void VK_image::create_image_sampler(Struct_image* texture){
  ...
  VkPhysicalDeviceProperties properties = vk_physical_device->find_device_property(vk_struct->device.physical_device);
  ...
  VkResult result = vkCreateSampler(vk_struct->device.device, &samplerInfo, nullptr, &texture->sampler);
  if(result != VK_SUCCESS){
...
void VK_image::create_image_obj(Struct_image* image){
  ...
  VkResult result = vkCreateImage(vk_struct->device.device, &image_info, nullptr, &image->image);
  if(result != VK_SUCCESS){
...
void VK_image::bind_image_to_memory(Struct_image* image){
  ...
  vkGetImageMemoryRequirements(vk_struct->device.device, image->image, &memRequirements);
  ...
  VkResult result = vkAllocateMemory(vk_struct->device.device, &allocInfo, nullptr, &image->mem);
  if(result != VK_SUCCESS){
    throw std::runtime_error("failed to allocate image memory!");
  }

  vkBindImageMemory(vk_struct->device.device, image->image, image->mem, 0);
