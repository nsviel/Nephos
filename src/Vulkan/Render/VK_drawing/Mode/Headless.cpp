...
Headless::Headless(vk::Structure* vk_struct) : vk::draw::Drawer(vk_struct){
  //---------------------------
...
void Headless::draw_frame(){
  ...
  for(auto& renderpass : vk_struct->render.renderpass.vec_renderpass){
    ...
    if(i < vk_struct->render.renderpass.vec_renderpass.size() - 1) command->semaphore_done = semaphore->handle;
