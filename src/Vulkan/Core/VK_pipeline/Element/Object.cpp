...
void Object::info_pipeline_vertex(vk::structure::Pipeline& pipeline){
  ...
  info_vertex.vertexAttributeDescriptionCount = static_cast<uint32_t>(pipeline.element.vec_vertex_attribute.size());
  info_vertex.pVertexBindingDescriptions = pipeline.element.vec_vertex_binding.data();
  info_vertex.pVertexAttributeDescriptions = pipeline.element.vec_vertex_attribute.data();
...
void Object::info_pipeline_depth(vk::structure::Pipeline& pipeline){
  ...
  info_stencil.depthTestEnable = (pipeline.shader.info.with_depth_test) ? VK_TRUE : VK_FALSE;
  info_stencil.depthWriteEnable = (pipeline.shader.info.with_depth_test) ? VK_TRUE : VK_FALSE;
