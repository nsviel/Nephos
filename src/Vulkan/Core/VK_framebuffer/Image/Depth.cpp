...
//Main function
void Depth::create_window_image(vk::structure::Image* depth){
  //---------------------------
...
void Depth::create_screenshot_image(vk::structure::Image* depth){
  //---------------------------
...
VkFormat Depth::find_supported_format(const std::vector<VkFormat>& format_candidates, VkImageTiling tiling, VkFormatFeatureFlags features){
  ...
  for(VkFormat format : format_candidates){
    ...
    vkGetPhysicalDeviceFormatProperties(vk_struct->core.device.physical_device.handle, format, &property);
