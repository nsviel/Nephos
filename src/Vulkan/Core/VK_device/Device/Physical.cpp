...
void Physical::compute_extent(){
  ...
  if(vk_struct->interface.param.headless){
    vk_struct->window.extent.width = vk_struct->interface.param.headless_dim.x;
    vk_struct->window.extent.height = vk_struct->interface.param.headless_dim.y;
  }
  else{
...
void Physical::rate_device_suitability(vk::device::structure::Physical& physical_device){
  ...
  if(physical_device.type == VK_PHYSICAL_DEVICE_TYPE_DISCRETE_GPU){
    ...
    if(vk_struct->interface.state.dedicated_gpu){
      score += 100000;
    }
    else{
      score -= 100000;
